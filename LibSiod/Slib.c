...
init_slib_version(void)
{
    setvar(cintern("*slib-version*"),
           cintern("$Id: slib.c,v 1.13 1997/06/06 12:56:19 gjc Exp $"),
           NIL);
}
...
char *tkbuffer = NULL;  // length == TKBUFFERN == 5120 (a mile?)
...
long siod_verbose_level = 4;    // brianbec: level 6 means single-step...
...
print_welcome(void)
{
    ...
    {
		char buf [128] ;
        sprintf(buf, "Welcome to SIOD, Scheme In One Defun, Version %s\n", 
			siod_version());
		SiodIPuts (buf) ;
        sprintf(buf, "(C) Copyright 1988-1994 Paradigm Associates Inc.\n");
		SiodIPuts (buf) ;
    }
}
...
print_hs_1(void)
{
    ...
    {
        char buf [128] ;
		sprintf(buf, "%ld heaps. size = %ld cells, %ld bytes. %ld inums. GC is %s\n",
               nheaps,
...
print_hs_2(void)
{
    if (siod_verbose_level >= 2)
    {
		...
		{
            sprintf (buf, "heaps[0] at %p, heaps[1] at %p\n", heaps[0], heaps[1]);
			SiodIPuts (buf) ;
		}
        else
		{
            sprintf (buf, "heaps[0] at %p\n", heaps[0]);
			SiodIPuts (buf) ;
		}
    }
}
...
#if defined(WIN32)
void
handle_interrupt_differed(void)
{
    interrupt_differed = 0;
    err_ctrl_c();
}
...
repl_driver(long want_sigint, long want_init, struct repl_hooks *h)
{
    ...
    if (want_sigint)
        osigint = signal(SIGINT, handle_sigint);
#ifdef WIN32_X
...
repl_c_string_read(void)
{
    ...
    if (repl_c_string_out)
        repl_c_string_out[0] = 0;
...
#if !defined(__osf__) & !defined(VMS) & !defined(SUN5) & !defined(WIN32)
double
myrealtime(void)
{
    time_t x;
...
// dangerous kludge for lprin1f
int
frepl_puts_fcn (char *st, void * pv)
{
	repl_puts (st) ;
	return 1 ;
}
...
err(const char *message, LISP x)
{
    ...
    if ((siod_verbose_level >= 1) && msg)
    {
		...
		{
            sprintf (buf, "ERROR: %s (errobj %s)\n", msg, eobj);
			SiodIPuts (buf) ;
		}
        else
		{
            sprintf (buf, "ERROR: %s (see errobj)\n", msg);
			SiodIPuts (buf) ;
		}
    }
    if (errjmp_ok == 1)
	...
	{
		char buf [128] ;
        sprintf (buf, "FATAL ERROR DURING STARTUP OR CRITICAL CODE SECTION\n");
		SiodIPuts (buf) ;
	}
    if (fatal_exit_hook)
        (*fatal_exit_hook) ();
...
plus(LISP x, LISP y)
{
    ...
    if (NFLONUMP(x))
        err("wta(1st) to plus", x);
    if (NFLONUMP(y))
        err("wta(2nd) to plus", y);
...
ltimes(LISP x, LISP y)
{
    ...
    if (NFLONUMP(x))
        err("wta(1st) to times", x);
    if (NFLONUMP(y))
        err("wta(2nd) to times", y);
    return (flocons(FLONM(x) * FLONM(y)));
}
...
{
    if (NFLONUMP(x))
        err("wta(1st) to difference", x);
    if (NULLP(y))
        return (flocons(-FLONM(x)));
    else
    {
        if (NFLONUMP(y))
            err("wta(2nd) to difference", y);
        return (flocons(FLONM(x) - FLONM(y)));
    }
}
...
{
    if (NFLONUMP(x))
        err("wta(1st) to quotient", x);
    if (NULLP(y))
        return (flocons(1 / FLONM(x)));
    else
    {
        if (NFLONUMP(y))
            err("wta(2nd) to quotient", y);
        return (flocons(FLONM(x) / FLONM(y)));
    }
}

LISP
IntegerQuotient(LISP x, LISP y)
{
	unsigned long a ;
    unsigned long b ;
	double        f = 0 ;
    ...
    if (NFLONUMP(x))
        err("wta(1st) to integer-quotient", x);
    if (NULLP(y))
        err ("integer-quotient must have two arguments", y) ;
    else
    {
		if (NFLONUMP(y))
            err("wta(2nd) to integer quotient", y);

		a = (unsigned long)FLONM(x) ;
		b = (unsigned long)FLONM(y) ;
...
LISP
symbol2string(LISP x)
{
    if (NSYMBOLP(x))
        err("not a symbol", x);

    return (strcons (strlen(PNAME(x)), PNAME(x)));
}

LISP 
string2symbol (LISP x)
{
    return intern (x) ;
}
...
init_storage_1(void)
{
    ...
    hex_mode = cintern("**hex-mode**");
    gc_protect(&hex_mode);
    setvar(hex_mode, sym_t, 0);
...
allocate_aheap(void)
{
    ...
    for (j = 0; j < nheaps; ++j)
        if (!heaps[j])
        {
			...
			{
				char buf [128] ;
                sprintf (buf, "[allocating heap %ld]\n", j);
				SiodIPuts (buf) ;
			}
...
gc_ms_stats_end(void)
{
	...
	{
		char buf [128] ;
        sprintf (buf, "[GC took %g cpu seconds, %ld cells collected]\n",
               gc_rt,
               gc_cells_collected);
		SiodIPuts (buf) ;
	}
...
loop:
    ...
    case tc_cons:
        ...
        case tc_subr_2:
            ...
            x = NULLP(x) ? NIL : CDR(x);
            return (SUBR2(tmp) (arg1, leval(car(x), env)));
...
setvar(LISP var, LISP val, LISP env)
{
    ...
    if (NSYMBOLP(var))
        err("wta(non-symbol) to setvar", var);
...
symbolconc(LISP args)
{
    ...
    for (l = args; NNULLP(l); l = cdr(l))
    {
        s = car(l);
        if (NSYMBOLP(s))
            err("wta(non-symbol) to symbolconc", s);
...
LISP
lispToString (LISP exp)
{
    struct gen_printio s;
    LISP   str = strcons (4096, "") ;
    extern int pts_puts (char *from, void *cb) ;
    ...
    str = strcons (strlen (STRING(str)), STRING(str)) ;
...
lprin1g(LISP exp, struct gen_printio * f)
{
    ...
    case tc_cptr:
        sprintf (tkbuffer, "#{CPTR 0x%X}", CPTR(exp)) ;
        gput_st (f, tkbuffer) ;
        break ;
    ...
    case tc_flonum:
        ...
        {
            if (EQ(VCELL(hex_mode), sym_t))
                sprintf(tkbuffer, "0x%X", n);
            else
...
//
// This one always prints a newline
//
LISP
...
//
// This one doesn't print a newline
//
LISP
...
//
// This one doesn't print a newline either
//
LISP
...
LISP
vload(char *ofname, long cflag, long rflag)
{
...
init_subrs_1(void)
{
	...
	init_subr_2n("quotient",           IntegerQuotient   ) ;
    ...
    init_subr_1 ("tostring",           lispToString      ) ;
    ...
    init_subr_1 ("symbol->string",     symbol2string) ;
    init_subr_1 ("string->symbol",     string2symbol) ;
