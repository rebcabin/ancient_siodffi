...
{setvar(cintern("*slibu-version*"),
        cintern("$Id: slibu.c,v 1.20 1997/05/16 08:03:25 gjc Exp $"),
        NIL);}
...
{struct rlimit x;
    ...
    {if (getrlimit(RLIMIT_CPU,&x))
            ...
            return (listn(2,flocons(x.rlim_cur),flocons(x.rlim_max)));} else
...
{struct rusage u;
    ...
    if (NULLP(kind) || EQ(cintern("SELF"),kind))
        code = RUSAGE_SELF;
    else ifEQ (cintern("CHILDREN"),kind)
...
{time_t bt;
    ...
    {sprintf(sbuff,"%04d%02d%02d%02d%02d%02d%02d",
        ...
        return (strcons(strlen(sbuff),sbuff));} else
        return (NIL);}
...
{time_t b;
    ...
    if ((buff = ctime(&b)))
    {if ((p = strchr(buff,'\n'))) *p = 0;
        return (strcons(strlen(buff),buff));} else
        return (NIL);}
...
LISP require_so(LISP fname)
{
    ...
    if (NULLP(symbol_boundp(init_name,NIL)) || NULLP(symbol_value(init_name,NIL)))
    {
        load_so(fname,NIL);
...
{char tmpbuff[1024],*p1,*p2;
    ...
    {strcpy(tmpbuff,"(require \"");
        ...
        *p1 = 0;
        strcat(tmpbuff,"\")");
        return (repl_c_string(tmpbuff,0,0,0));} else
        return (repl_c_string(value,0,0,0));}

int ourPuts (const char * s)
{
    return fputs (s, stdout) ;
}

int __stdcall siod_main(int argc,char **argv, char **env, int (*putsHook) (const char *))
{
