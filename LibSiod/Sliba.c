...
static void init_sliba_version(void)
{
    setvar(cintern("*sliba-version*"),
           cintern("$Id: sliba.c,v 1.6 1997/05/05 15:38:25 gjc Exp $"),
           NIL);
}
...
void init_storage_a(void)
{
    ...
    init_storage_a1(tc_cptr_array) ;  // consider adding cpp arrays that are "deleted"
...
/** /   {
            int i ;
            for (i = 0 ;  i < ptr->storage_as.cptr_array.dim ;  i ++)
            {
                // UNDONE: "free", "LocalFree", "delete", what?
            }
        }
/**/    free(ptr->storage_as.cptr_array.data);
        break;
...
void string_write1 (LISP ptr, struct gen_printio *f)
{
    switch (ptr->type)
    {
    case tc_string:
        gput_st (f, STRING(ptr)) ;
        break;

    default:
        err ("wta string_write", ptr) ;
        break ;
    }
}

LISP write_string (LISP str)
{
	struct gen_printio s ;
    extern void (*repl_puts ) (char *) ;
    extern int    frepl_puts_fcn (char *st, void * pv) ;
    extern int    fputs_fcn      (char *st, void *cb)  ;

    s.putc_fcn = NULL;

	if (repl_puts)
		s.puts_fcn = frepl_puts_fcn;
	else
		s.puts_fcn = fputs_fcn;
	
	s.cb_argument = stdout;
...
void array_prin1 (LISP ptr, struct gen_printio *f)
{
    ...
    case tc_cptr_array:
        gput_st(f,"#(");
        for (j=0; j < ptr->storage_as.cptr_array.dim; ++j)
        {
            sprintf(tkbuffer,"0x%X",ptr->storage_as.cptr_array.data[j]);
            gput_st(f,tkbuffer);

            if ((j + 1) < ptr->storage_as.cptr_array.dim)
                gput_st(f," ");
        }
        gput_st(f,")");
        break;
...
LISP filledstring (LISP n, LISP a)
{
    long k;
    char * s = 0 ;
    char * t = 0 ;
    ...
    {
        if (NFLONUMP (n))
            err("bad index to string",n);

        k = (long) FLONM(n);

        if (k < 0)
            err("negative index to string",n);
    }
...
LISP aref1(LISP a,LISP i)
{
    ...
    case tc_cptr_array:
        if (k >= a->storage_as.cptr_array.dim) err("index too large",i);
        return conscptr(a->storage_as.cptr_array.data[k]);
...
LISP aset1(LISP a,LISP i,LISP v)
{
    ...
    case tc_cptr_array:
        if (k >= a->storage_as.cptr_array.dim) err1_aset1(i);
        a->storage_as.cptr_array.data[k] = CPTR(v) ;
        return (v);
...
LISP arcons(long typecode,long n,long initp)
{
    ...
    case tc_double_array:
        ...
        a->storage_as.double_array.data = (double *) must_malloc(n *
                                                                 sizeof(double));
        if (initp)
            memset (a->storage_as.double_array.data, 0, sizeof(double)*n) ;
    ...
    case tc_long_array:
        ...
        if (initp)
            memset (a->storage_as.long_array.data, 0, sizeof(long)*n) ;
    ...
    case tc_string:
        ...
        if (initp)
            memset (a->storage_as.string.data, ' ', n) ;

    case tc_byte_array:
        a->storage_as.string.dim = n;
        a->storage_as.string.data = (char *) must_malloc(n);
        if (initp)
            memset (a->storage_as.string.data, 0, n) ;
        break;
    ...
    case tc_cptr_array:
        ...
        for (j=0;j<n;++j) 
            a->storage_as.cptr_array.data[j] = NIL;
        break;
...
/**/else if (EQ(cintern("cptr"),kind))
    {
        ...
        a->storage_as.cptr_array.dim = n;
        a->storage_as.cptr_array.data = (void**) must_malloc(n * sizeof(void*)) ;
        memset (a->storage_as.cptr_array.data, 0, n * sizeof(void*)) ;
        //for (j=0;j<n;++j) a->storage_as.lisp_array.data[j] = NIL;
    }
/**/else if (EQ(cintern("lisp"),kind) || NULLP(kind))
    {
        ...
        a->storage_as.lisp_array.dim = n;
        a->storage_as.lisp_array.data = (LISP *) must_malloc(n * sizeof(LISP));
        memset (a->storage_as.lisp_array.data, 0, n * sizeof(LISP)) ;
        //for (j=0;j<n;++j) a->storage_as.lisp_array.data[j] = NIL;
    }
    else
...
LISP array_equal(LISP a,LISP b)
{
    ...
    case tc_cptr_array:
        len = a->storage_as.cptr_array.dim;
        if (len != b->storage_as.cptr_array.dim) 
            return (NIL);
        for (j=0;j<len;++j)
            if (a->storage_as.cptr_array.data[j] != 
...
long array_sxhash(LISP a,long n)
{
    ...
    case tc_cptr_array:
        len = a->storage_as.cptr_array.dim;
        for (j=0, hash=0, cptr_data=a->storage_as.cptr_array.data;
            j < len;
            ++j,++cptr_data)
            hash = HASH_COMBINE (hash, ((unsigned long)(*cptr_data)) % n, n);
...
{
    long index;
    if (NTYPEP (table,tc_lisp_array))
        err("not a hash table",table);
...
LISP array_fast_print(LISP ptr,LISP table)
{
    ...
    case tc_long_array:
        ...
        return (NIL);
    case tc_cptr_array:
        putc(tc_cptr_array,f);
        len = ptr->storage_as.cptr_array.dim;
        put_long(len,f);
        for (j=0; j < len; ++j)
            fast_print(ptr->storage_as.cptr_array.data[j],table);
        return (NIL);
    case tc_lisp_array:
...
{long j,len,iflag;
    ...
    case tc_cptr_array:
        len = get_long(f);
        iflag = no_interrupt(1);
        ptr = newcell(tc_cptr_array);
        ptr->storage_as.cptr_array.dim = len;
        ptr->storage_as.cptr_array.data =
        (void**) must_malloc(len * sizeof(void*));
        fread(ptr->storage_as.cptr_array.data,sizeof(void*),len,f);
        no_interrupt(iflag);
        return (ptr);
    case tc_lisp_array:
        len = get_long(f);
...
long nlength(LISP obj)
{
    ...
    case tc_cptr_array:
        return (obj->storage_as.cptr_array.dim);
    case tc_lisp_array:
...
{
    if (NFLONUMP (x))
        err("wta(1st) to fmod",x);
    if (NFLONUMP (y))
        err("wta(2nd) to fmod",y);
...
{
    if (NFLONUMP (x))
        err("wta(1st) to pow",x);
    if (NFLONUMP (y))
        err("wta(2nd) to pow",y);
...
LISP asciistr(LISP a)
{
    long dim ;
    return strcons (-1, get_c_string_dim(a, &dim));
}
...
LISP ascii2bytes(LISP a, LISP moreroom)
{
    char          *in        ;
    unsigned char *out       ;
    LISP           result    ;
    ...
    if (NNULLP(moreroom) && TYPE(moreroom)==tc_flonum)
    {
        more = (long)FLONM(moreroom) ;
        more = (more > dim) ? more : dim ;
    }

    result = arcons(tc_byte_array,more,1) ;
...
LISP ltypeof(LISP obj)
{
    ...
    case tc_cptr_array:   return (cintern("tc_cptr_array"  )) ;
    case tc_lisp_array:   return (cintern("tc_lisp_array"  )) ;
    case tc_c_file:       return (cintern("tc_c_file"      )) ;
    case tc_cptr:         return (cintern("tc_cptr"        )) ;

    default:              return (flocons(x));
    }
}

LISP caaar(LISP x)  {return (car(car(car(x))));}
LISP caadr(LISP x)  {return (car(car(cdr(x))));}
LISP cadar(LISP x)  {return (car(cdr(car(x))));}
LISP caddr(LISP x)  {return (car(cdr(cdr(x))));}
LISP cdaar(LISP x)  {return (cdr(car(car(x))));}
LISP cdadr(LISP x)  {return (cdr(car(cdr(x))));}
LISP cddar(LISP x)  {return (cdr(cdr(car(x))));}
LISP cdddr(LISP x)  {return (cdr(cdr(cdr(x))));}
...
void init_subrs_a(void)
{
    ...
    init_subr_2("string",            filledstring) ;
    ...
    init_subr_1("array->string",      asciistr) ;
    init_subr_2("string->array",      ascii2bytes) ;
    init_subr_1("write-string",       write_string) ;
	...
	init_subr_2("bit-shl",            bitshl);
	init_subr_2("bit-shr",            bitshr);
